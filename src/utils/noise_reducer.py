...
def reduce_noise(image: np.ndarray, strength: int = 10, color_strength: int = 10, template_window_size: int = 7, search_window_size: int = 21) -> np.ndarray:
    ...
    Args:
        ...
        strength: Strength of the denoising filter (for luminance). Higher values mean stronger denoising (default: 10).
        color_strength: Strength of the color denoising filter. A higher value results in stronger color denoising (default: 10).
